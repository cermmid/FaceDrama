...
2. Skopiuj `ArkitBlendshapeMap.cs`, `MicAudioSource.cs`, `BlinkAndIdle.cs`,
   `IdleBodyMotion.cs`, `LipSyncArkitApplier.cs` do `Assets/Scripts/`, a
   `Editor/FaceDramaSetupWizard.cs` do `Assets/Scripts/Editor/`
   (nazwa folderu **Editor** jest obowiązkowa).
...
| `IdleBodyMotion.cs` | 1+ | — (czysty Unity) |
